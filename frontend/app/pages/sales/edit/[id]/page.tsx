...
export default function Form({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        ...
        const urlSales = `http://localhost:8080/api/sales/view/${id}`; 
        
        
        const formData = {};
        axios.get(urlSales, { headers, withCredentials: true })
        .then(response => {
            const datasales = response.data;
            setSales(datasales.sales);
            const url = `http://localhost:8080/api/employees/view/${datasales.employeeId}`;
            axios.get(url, { headers, withCredentials: true })
                .then(respon => {
                    const datapegawai = respon.data;
                    const tgl = datapegawai.joined_date ? new Date(datapegawai.joined_date) : null;
                    const monthlist = ['01','02','03','04','05','06','07','08','09','10','11','12'];
                    const join = (tgl?.getFullYear() ?? '2024') + '-' + monthlist[tgl?.getMonth() ?? 0] + '-' + (tgl?.getDate() ?? '01');
                    setEmployeeid(datapegawai.employee_id ?? '');
                    setName(datapegawai.name ?? '');
                    setJobtitle(datapegawai.job_title ?? '');
                    setSalary(datapegawai.salary ? datapegawai.salary.toString() : '0');
                    setDepartment(datapegawai.department ?? '');
                    setJoindate(join);
                });
        });
    ...
    async function onSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        const url = `http://localhost:8080/api/sales/update/${id}`;
        const headers = {
          'Content-Type': 'application/json',
        };
        const formData = {
  ...
  return (
    <div>
      <h1 className="text-2xl">Sales Form Edit</h1>
      <div className="card">
